...
import {
    ...
    PaginationParams as PaginationParamsInterface,
    SortParams as SortParamsInterface,
    TagsParams as TagsParamsInterface,
...
const Blogs = <T,>({
    ...
    const [sortValue, setSortValue] = useState<SortKeyDirectionType>((decodedQuery.current?.sort ?? ['title', DESC]) as SortKeyDirectionType)
    ...
    // const [paginator, setPaginator] = useState<PaginationParamsInterface>((() => {
    //     return { ...pagination, page: parseInt((decodedQuery.current?.page ?? pagination.page) as string, 10) }
    // })())
    // const [sorter, setSorter] = useState<SortParamsInterface<T>>((() => {
    //     return { ...sort, active: (decodedQuery.current?.sort ?? sort.active) as SortKeyDirectionType }
    // })())
    // const [searcher, setSearcher] = useState<SearchParamsInterface>((() => {
    //     return { ...search, text: decodedQuery.current?.search ?? search.text }
    // })())
    // const [tagger, setTagger] = useState<TagsParamsInterface>((() => {
    //     return { ...tags, active: decodedQuery.current?.tags ?? tags.active }
    // })())
    ...
    const filteredBlogs = useMemo<BlogItemInterface<T>[]>(() => {
        ...
        if (sort.keys.length > 0) {
            const [sKeys, sDirection] = sortValue
